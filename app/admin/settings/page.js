...
export default function SettingsPage() {
  ...
  const fetchBanner = async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/promobanner`);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (bannerId) {
        await axios.put(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/promobanner/${bannerId}`,
          { text },
        ...
        const res = await axios.post(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/promobanner`,
          { text },
          { headers: { Authorization: `Bearer ${token}` } }
        );
