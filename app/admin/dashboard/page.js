...
const navItems = [
  ...
  { name: "ORDERS", href: "/admin/orders/websiteOrders", icon: ShoppingBag },
];
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    if (!token) return router.push('/account/login');
    try {
    ...
    } catch (e) { router.push('/account/login'); }
  }, [router]);
    ...
    <div className="min-h-screen w-full bg-[#fdfdfd] relative overflow-x-hidden pb-20">
      ...
      <header className="w-full bg-white/80 backdrop-blur-xl border-b border-gray-100 sticky top-0 z-50">
        <div className="w-full px-6 py-4 flex flex-wrap items-center justify-between gap-4">
          ...
          <nav className="flex items-center bg-gray-100/80 p-1.5 rounded-2xl border border-gray-200/30">
            {navItems.map((nav) => (
              <Link key={nav.href} href={nav.href} className={`px-5 py-2 rounded-xl text-[10px] font-black transition-all tracking-widest ${pathname === nav.href ? "bg-white text-black shadow-sm" : "text-gray-400 hover:text-black"}`}>
                {nav.name}
              </Link>
            ))}
          </nav>
      ...
      <motion.main variants={containerVariants} initial="hidden" animate="show" className="w-full p-6 lg:p-10 space-y-10 relative z-10">
        ...
        <div className="grid grid-cols-1 xl:grid-cols-4 gap-8">
          ...
          <motion.section variants={itemVariants} className="bg-white border border-gray-100 p-8 rounded-[3rem] shadow-sm h-fit">
            ...
            <Link href="/admin/allproduct" className="mt-12 w-full py-5 border border-dashed border-gray-200 rounded-3xl flex items-center justify-center gap-3 text-[9px] font-black uppercase tracking-[0.2em] text-gray-400 hover:border-black hover:text-black transition-all group">
