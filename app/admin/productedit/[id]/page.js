...
export default function EditProductPage() {
  ...
  const fetchOptions = useCallback(async () => {
    try {
      const [b, t, col, c] = await Promise.all([
        axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/brand`),
        axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/tax`),
        axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/collection`),
  ...
  const fetchProduct = useCallback(async () => {
    try {
      const res = await axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/product/${id}`);
      const data = res.data;
      ...
      <header className="w-full bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="w-full px-6 py-4 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Link href="/admin/allproduct" className="p-2 hover:bg-gray-100 rounded-full transition-colors">
              <ArrowLeft size={20} />
            </Link>
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 xl:grid-cols-12 gap-8 items-start">
          ...
          <div className="xl:col-span-4 space-y-8">
            ...
            <div className="sticky bottom-6 flex flex-col gap-3">
              ...
              <Link href="/admin/allproduct" className="w-full py-4 text-center text-gray-400 font-bold hover:text-black transition-colors text-sm">Discard Changes</Link>
            </div>
