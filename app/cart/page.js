...
useEffect(() => {
    const fetchCart = async () => {
      try {
        ...
        const cartRes = await axios.get(
          `${process.env.NEXT_PUBLIC_BACKEND_URL}/cart/specificcart/${cartId}`
        );
  ...
  const updateCartToPending = async (cartid) => {
    try {
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/cart/update-status/${cartid}`,
        {
          method: "PUT",
  ...
  const handleDeleteItem = async (item) => {
    try {
      ...
      await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/cart/deleteproductcart/${cartId}`,
        {
  ...
  const handleUpdateItemQty = async (item, newQty) => {
    try {
      ...
      await axios.put(
        `${process.env.NEXT_PUBLIC_BACKEND_URL}/cart/editproductcart/${cartId}`,
        {
          product: item.productId,
